    ...
    border-radius: 15px;
        ...
        $(function () {
            $('.form').submit(function () {
                $.ajax({
                    <?= "url: 'codEditarCursos.php?codCurso=$codCurso'," ?>
                    type: 'POST',
                    data: $('.form').serialize(),
            ...
            <main>
                <div class="acessoUm">
                    ...
                    <?php
                        if(isset($_REQUEST['codCurso'])){
                    ?>
                    <h1>Atualizar o curso:</h1>
                    
                    <?php
                        $selectCurs = $pdo->prepare("select nome_curso from cursos where cod_curso = $codCurso");
                        $selectCurs->execute();
                        $select = $selectCurs->fetch(PDO::FETCH_ASSOC);

                        $nomeCurso = $select['nome_curso'];
                    ...
                    <form class='form' method='post'>
                        ...
                        <input class='unid' id='IdSigla0' name='nome_curso' type='text' <?="value='$nomeCurso'"?>/>

                        <div id="rightDiv"></div> <!-- div q recebe os novos inputs -->
                        <div class='recebeDados' id='div'></div> <!-- div que recebe dados do ajax -->
                        <div class="puto"><input type="submit" value="Atualizar Curso" class="VAISEFUDE" /></div> 
                    </form>
                    <?php
                        }else{
                            echo "<p>Você não escolheu nenhum curso, volte e escolha qual curso você quer atualizar</p>";
                        }
                    ?>
