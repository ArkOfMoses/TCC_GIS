...
if(isset($_REQUEST['codTurma']) && isset($_REQUEST['codDisc'])){
    ...
    border-radius: 15px;
        ...
        $(function () {
            $('.form').submit(function () {
                $.ajax({
                    <?= "url: 'codEditarDisc.php?codDisc=$codDisc'," ?>
                    type: 'POST',
                    data: $('.form').serialize(),
    ...
    <body>
        <div class="content">
            ...
            <main>
                <div class="acessoUm">
                    <div class="setinha">
                        <?php echo "<a href='../verDisc.php?codTurma=$codTurma'>";?>
                    ...
                    <?php
                        if(isset($_REQUEST['codTurma']) && isset($_REQUEST['codDisc'])){
                    ?>
                    <h1>Atualizar a Disciplina:</h1>
                    
                    <?php
                        $selectCurs = $pdo->prepare("select nome_disc, carga_horaria_disc from disciplina where cod_disc = $codDisc");
                        $selectCurs->execute();
                        $select = $selectCurs->fetch(PDO::FETCH_ASSOC);

                        $nomeDisc = $select['nome_disc'];
                        $cargaHoraria = $select['carga_horaria_disc'];
                    ...
                    <form class='form' method='post'>
                        ...
                        <input class='unid' id='IdSigla0' name='nome_curso' type='text' <?="value='$nomeDisc'"?>/>

                        <label id="sigla_label">Carga Horaria da Turma: </label>
                        <input class='unid' id='IdSigla0' name='carga_horaria' type='text' <?="value='$cargaHoraria'"?>/>

                        <div id="rightDiv"></div> <!-- div q recebe os novos inputs -->
                        <div class='recebeDados' id='div'></div> <!-- div que recebe dados do ajax -->
                        <div class="puto"><input type="submit" value="Atualizar Disciplina" class="VAISEFUDE" /></div> 
                    </form>
                    <?php
                        }else{
                            echo "<p>Você não escolheu nenhuma disciplina, volte e escolha qual disciplina você quer atualizar</p>";
                        }
                    ?>
